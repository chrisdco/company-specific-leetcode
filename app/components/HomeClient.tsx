...
export default function HomeClient({
  ...
  // Initials arrive from the server (URL params + theme cookie), so SSR HTML
  // and the first client render agree by construction. No lazy window reads,
  // no suppressHydrationWarning, no mismatch_regeneration cycle.
  ...
  // Persist theme where the server can read it on the next load (cookie, not
  // localStorage - the server renders the theme, so it must see the value).
  useEffect(() => {
    try {
      document.cookie = `csl-theme=${theme}; path=/; max-age=31536000; SameSite=Lax`;
    } catch { /* ignore */ }
  }, [theme]);

  // Flip themes without the full-tree color crossfade that made switches feel
  // stuck: transitions off, paint, transitions back on.
  const applyTheme = useCallback((next: ThemeName) => {
    try {
      const el = document.documentElement;
      el.classList.add("theme-switching");
      setTheme(next);
      const clear = () => el.classList.remove("theme-switching");
  ...
  const loadCompanyList = useCallback(async () => {
    ...
    try {
      ...
      // Freshness dates load lazily and non-blocking through their own
      // endpoint, so a slow commits call can never hold up this list.
      void fetch("/api/freshness")
        .then((r) => (r.ok ? r.json() : null))
        .then((d: unknown) => {
          if (d && typeof d === "object") {
            const f = d as { primary?: unknown; fallback?: unknown };
            setFreshness({
              primary: typeof f.primary === "string" ? f.primary : null,
              fallback: typeof f.fallback === "string" ? f.fallback : null,
            });
          }
        })
        .catch(() => {
          /* best-effort: the source line simply omits the date */
        });
    } catch (e) {
  ...
  const lastResultsRef = useRef<LoadOk[]>([]);
  const autoloaded = useRef(false);
  const [failedCompanies, setFailedCompanies] = useState<string[]>([]);
  const [cacheNote, setCacheNote] = useState<number | null>(null);
  ...
  // Cache hydration happens here (not in render) so SSR and hydration agree;
  // the cached paint is instant, then the fresh fetch replaces it.
  useEffect(() => {
    ...
    try {
      ...
      if (sp.get("company") && sp.get("autoload") !== "0" && companies.length > 0 && !loading) {
        ...
        const t = setTimeout(() => {
          if (cancelled) return;
          const hit = readCachedResults();
          if (hit) {
            lastResultsRef.current = hit.ok;
            setProblems(hit.problems);
            setSource(hit.source);
            setLastFetched(hit.snapshot);
            setHasSearched(true);
            setCacheNote(hit.at);
          }
          void fetchProblems();
